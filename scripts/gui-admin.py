...
#                 messagebox.showerror(
#                     "Error",
#                     "Please enter both GitHub username and password/token.",
#                     parent=dialog
#                 )
#                 return
...
#         if confirm:
#             username, password = self.ask_github_credentials()
...
#                 #["git", "push"],
#                 ["git", "push", "https://github.com/Isha-Arcubus/Arcubus-New.git"],
#                 cwd=REPO_ROOT,
#                 capture_output=True,
#                 text=True,
#                 input=f"{username}\n{password}\n"
...
#         askpass_file = Path(tempfile.gettempdir()) / "arcubus_git_askpass.cmd"

#         askpass_file.write_text(
#             f'@echo off\n'
#             f'if "%1"=="Username for*" echo {username}\n'
#             f'if "%1"=="Password for*" echo {password}\n',
#             encoding="utf-8"
#         )
...
- Replace article images
- Manage Our People
- Token-based Git publish/deployment
"""
...
if getattr(sys, "frozen", False):
    ...
    #   config.txt
    #   site/
    #   people_manager.py / bundled equivalent
    #
    REPO_ROOT = Path(sys.executable).resolve().parent
    SITE_DIR = REPO_ROOT / "site"
...
class ArcubusAdminApp:
    ...
    def update_image_preview(
        ...
        if image_path:
            try:
                ...
                if image_path_obj.exists() and PIL_AVAILABLE:
                    loaded_image = Image.open(
                        image_path_obj
                    )
    ...
    def process_article(self):
        ...
        if not desc:
            desc = (
                f"{title} — practical guidance "
                "from Arcubus Advisors."
            )
        ...
        if knowledge_centre_path.exists():
            ...
            # -------------------------------------------------
            # IMPORTANT:
            # Replace this marker with the actual marker
            # present in knowledge-centre.html if different.
            # -------------------------------------------------
            ...
            new_article_card = (
                f'\n'
                f'<a href="knowledge/{slug}.html" '
                f'class="knowledge-card">\n'
                f'    <div class="knowledge-card-category">'
                f'{safe_category}'
                f'</div>\n'
                f'    <h3>{safe_title}</h3>\n'
                f'    <p>{safe_lede}</p>\n'
                f'    <div class="knowledge-card-meta">'
                f'{display_date} · {safe_read_time}'
                f'</div>\n'
                f'</a>\n'
            )
            ...
            if not article_already_exists:
                ...
                if article_grid_marker in knowledge_centre_html:
                    ...
                    messagebox.showwarning(
                        "Knowledge Centre",
                        "Article grid marker was not found.\n\n"
                        "The article HTML was created, but "
                        "the Knowledge Centre listing was not "
                        "automatically inserted.\n\n"
                        "Please verify knowledge-centre.html."
                    )
        ...
        messagebox.showinfo(
            ...
            f"{out_path.name}\n\n"
            "Knowledge Centre update completed."
        )
    ...
    def open_edit_session(self):
        ...
        if (
            ...
            messagebox.showerror(
                "Error",
                "Please select a valid HTML article."
            )
            return False
    ...
    def apply_typo_correction(self):
        ...
        if position == -1:
            messagebox.showwarning(
                "Not Found",
                f"Specified text was not found:\n\n"
                f"{typo}"
            )
            return
    ...
    def save_final_edits(self):
        if (
            ...
            messagebox.showinfo(
                "Saved",
                "All changes were written successfully:\n"
                f"{self.active_file_path.name}"
            )
    ...
    def trigger_git_engine(self):
        ...
        if not token or not user or not repo:
            messagebox.showerror(
                "Credentials Missing",
                "Missing token configuration parameters "
                "inside config.txt.\n\n"
                "Please make sure config.txt contains:\n"
                "GITHUB_TOKEN=...\n"
                "GITHUB_USER=...\n"
                "GITHUB_REPO=..."
            )
        ...
        confirm = messagebox.askyesno(
            ...
            "The application will stage, commit and push "
            "the current website changes to GitHub."
        )
        ...
        try:
            ...
            if check_repo.returncode != 0:
                raise Exception(
                    "The application directory is not "
                    "inside a Git repository."
                )

            # =================================================
            # 2. SAVE ORIGINAL REMOTE
            # =================================================
            ...
            # =================================================
            # 6. CREATE AUTHENTICATED REMOTE
            # =================================================

            authenticated_url = (
                f"https://{user}:{token}"
                f"@github.com/{user}/{repo}.git"
            )
            ...
            if remote_set_result.returncode != 0:
                raise Exception(
                    remote_set_result.stderr.strip()
                    or "Could not configure authenticated remote."
                )
            ...
            messagebox.showinfo(
                "Published Successfully",
                "Changes successfully synced with GitHub.\n\n"
                "The Vercel deployment pipeline should "
            ...
            # =================================================
            # ALWAYS TRY TO RESTORE ORIGINAL REMOTE
            # =================================================
            ...
            messagebox.showerror(
                "Deployment Failed",
                "Pipeline encountered an operational error:\n\n"
                f"{e}"
            )
